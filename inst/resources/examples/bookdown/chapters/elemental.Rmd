...
(1) の方法が現在最も簡単である. ただし, `r rmdja::texlogo("LaTeX")` LaTeX の構文が評価されなくなるため同時に使うことはできない. 例えば太字強調と数式を両方表示したい場合は, `knitr::is_latex_output()` PDF の場合は完全に `r rmdja::texlogo("LaTeX")` で, HTML の場合は Markdown で書く, という場合分けを自分で書いて `knitr::kable()` に与えなければならない(表 \@ref(tab:kable-caption-example)). 
