...
さらに, 定理 \@ref(thm:thorem1) がある.

```{theorem thorem1, name="TODO の定理"}
TODO
```

## カスタムブロック

数式のセクションの定理ブロックの応用で, 独自のブロックセクションを定義することができる. `rmdja` では (ref:BKD)  [Ch. 2.7 Custom blocks](https://bookdown.org/yihui/bookdown/custom-blocks.html) で紹介されている例を予め使えるようにしている. それらは `type="..."` で指定できて, 以下の5種類がある.
...
それ以外にも, 相互参照と似たような構文でマクロ的にテキストを置き換えることができる. 例えばここまで何度も (ref:BKD) と (ref:RDG) に言及してきたが, 毎回ハイパーリンクと強調フォントにするのは面倒である. そこで, `(ref:RDG) [**RDG**](https://bookdown.org/yihui/rmarkdown/)` のような宣言を前後に改行を入れて書くことでトークンを作成できる

後は本文中に `(ref:RDG)` と書くだけでリンクが作られる.

別の利用方法として, チャンクオプションの `fig.cap` などに TeX 数式を書いても正しく表示できない. そのような場合は `ref` 参照を使う. `(ref:figcap1) \coloremoji{🌸} $\sum \oint \mathfrak{A} \mathscr{B} \mathbb{C}$ \coloremoji{🌸}` と書くと, 図 \@ref(fig:caption) のキャプションにも特殊な記号が使える.
