---
title: "R Markdownで日本語\\texttt{beamer}プレゼンテーション (XeLaTeX)版"
...
```{r template-selection-image, out.width="80%"}
...
* フォントがOSごとに異なるのが問題
* このファイルの設定なら Linux / Win / Mac のいずれでも動く?
...
jfontpreset: "`r switch(Sys.info()['sysname'], 'Linux' = 'noto', 'Darwin' = 'yu-osx', 'Windows' = 'ms')`"
```
...
file.copy(file.path(file_loc, "latex/tab.tex"), "./")
```
...
* 対応フォント: `{\CJKfamily{Noto Serif CJK JP} Noto}`{=latex}, `{\CJKfamily{IPAMincho} IPA}`{=latex}, `{\CJKfamily{Harano Aji Mincho} 原ノ味}`{=latex}, ヒラギノ, 游, モリサワ, 小塚, MS など
  + プリセットにないものは手動設定
...
* 詳細: [\XeLaTeX](http://ctan.math.washington.edu/tex-archive/language/japanese/zxjafont/zxjafont.pdf), [\LuaLaTeX](https://ja.osdn.net/projects/luatex-ja/wiki/LuaTeX-ja%E3%81%AE%E4%BD%BF%E3%81%84%E6%96%B9)
...
|MS       |`ms`      |`ms`|
|Noto     |`noto`/`noto-jp`|`noto-otf`/`noto-otc`|

* 游書体はOSバージョンごとに異なる
  + `ym-win`, `yu-win10`, `yu-osx`
* Ubuntu で Noto を選ぶ場合は `noto` で OK
* 上記以外にも多数
...
```md
ここはいつものフォント.
`\textrm{ここだけ明朝 Mincho}`{=latex}
```

ここはいつものフォント.`\textrm{ ここだけ明朝 Mincho.}`{=latex}
...
* `natbib`
  + [`jecon.bst`](https://github.com/ShiroTakeda/jecon-bst) が使える
  + **TeXLiveが必要な原因**
* `biblatex`(+`biber`)
  + 有力な日本語フォーマットがない?
  + **TeXLive不要**
* `citeproc`: `pandoc-citeproc`の機能
  + `csl`ファイルで参考文献リストの体裁指定
  + **TeXLive不要**
...
* A2. 書く場所を間違っている
  + yamlヘッダの入れ子には意味がある.
  + すこしややこしいので[解説](https://ill-identified.hatenablog.com/entry/2020/09/05/202403)
...
* **悪化**したこと
  + 数式のリアルタイム補完はLyXが依然優秀
  + python作業中(jupyterへの)**不満高まり**
