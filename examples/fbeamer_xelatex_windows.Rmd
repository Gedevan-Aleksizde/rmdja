...
* 以下, 表を一旦`.tex`ファイルで出力してから読み込む
  + R上で生成したTeXコードなら**直接出力可**(次のスライド参照)
  + Windowsでテキストファイルを書き出し再読み込みする場合は**必ずUTF-8に**
...
* 本文中の一部だけフォントを変更したい時は `\CJKfamily{}` を使う
* 欧文なら `\fontspec{}` に置き換える
* 詳しくは[ここ](http://zrbabbler.sp.land.to/xelatex.html#sssec-newfont)

```md
ここはいつものフォント.
`\textrm{\CJKfamily{IPAMIncho} ここだけIPA明朝.}`{=latex}
```

ここはいつものフォント.`\textrm{\CJKfamily{IPAMincho} ここだけIPA明朝.}`{=latex}
