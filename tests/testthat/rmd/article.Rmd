...
```{r plot, fig.cap="(ref:ggplot-example-article)", warning=F}
with(mtcars, plot(mpg, wt, col = factor(cyl),
                  xlab = "ガロン毎マイル", ylab = "重量 (1000ポンド)"))
```

```{r plot2, fig.cap="(ref:ggplot-example-article)", dev="svglite", warning=F}
par(family = setNames(rmdja::get_default_font_family()["serif"], ""))
with(mtcars, plot(mpg, wt, col = factor(cyl),
                  xlab = "ガロン毎マイル", ylab = "重量 (1000ポンド)", main = "cairo_pdf"))
```
